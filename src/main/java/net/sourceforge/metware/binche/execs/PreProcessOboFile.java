 ...
 * 6) metadata properties to carry over to new ontology
 * (semicolon-separated)</br>
 * 7) [optional] relationship(s) to infer over (semicolon-separated) for
 * annotation file and inherited inferences</br>
 * 
...
public class PreProcessOboFile {
    ...
    /**
     * Main to run the task of pre-processing the obo file, with the following
     * parameters:
     *     
* 1) obo file source 2) obo file output 3) boolean
     * carryDownInferredRelations 4) boolean writeSeparateAnnotationFile 5) root
     * ontology id(s) to extract subtrees from (semicolon-separated) 6) metadata
     * properties to carry over to new ontology (semicolon-separated) 7)
     * (optional) relationship(s) to carry inferences with and/or create
     * separate annotations with (semicolon-separated)
     *     
* @param args
     */
    ...
    /**
     * This method loads an ontology, runs the reasoner and builds a new
     * ontology with: - all subclassOf relations - for each class inherited
     * properties are "re-assigned" directly - for the properties specified in
     * the list, the class of the object property is followed up - all relations
     * are restricted to classes belonging to the subtrees of the classes passed
     * as parameters in chebiIDsForSubtrees; any other class is simply ignored
     *     
* It aims to address the following task: compute the transitive closure of
     * the has-role (and other, but mostly has-role) relationship over the is-a
     * relationship.
     *     
* EXAMPLE Input: Amphetamines hasRole CNSStimulant Mephedrome isA
     * Amphetamines Result: Amphetamines hasRole CNSStimulant Mephedrome isA
     * Amphetamines Mephedrome hasRole CNSStimulant
     *     
* @param ontologyIRI path to the ontology file
     * @param newOntIRI obo file OUTPUT
     * @param carryDownInferredRelations
     * @param writeSeparateAnnotationFile true if a separate annotation file
     * needs to be written.
...
// Save the new ontology
    ...
    private void postProcess(String oboFile, String outputFile) {
        /**
         * We need to change some things in the file we created with
         * getTransitiveClosure(...) - IDs are changed into CHEBI_123 and don't
         * match the original notation any more (CHEBI:123) - The "is-a Thing"
         * assertions must be deleted too
         */
        try {
