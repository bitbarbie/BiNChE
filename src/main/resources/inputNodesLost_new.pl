...
while($testfile = <$infile>) {
    ...
    eval {
        open($out, substr($result,0,29)."saddle/dot/".substr($result,29,length($result)).".dot") or die print "out ".substr($result,0,29)."saddle/dot/".substr($result,29,length($result)).".dot geht ni\n";
        %results;
        while($line = <$out>) {
            @splits = split(/ /,$line);
            if( $splits[3] =="["){
